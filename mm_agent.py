...
class InputAgent:
    ...
    def run(self,article:dict):
        ...
        print ("input agent running",article.keys())
...
class StateMachine:
    def __init__(self,api_key=None,st=None):
        import os
        ...
        self.st=st #way back to streamlit for debugging
    ...
    def resume(self,new_values:dict):
        ...
        self.chain.update_state(self.thread,values)
        result=self.chain.invoke(None,self.thread)
        if result is None:
            last_state=self.chain.get_state(self.thread).next[0].split(':')[0]
